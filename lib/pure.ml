...
(** 
   Converte uma lista de strings para um record de [order].
...
(** 
   Converte uma lista de strings para um record de [order_item].
...
(** 
   ...
   - Imposto: (tax / 100.0) * (quantidade * preço).
*)
...
(** 
   Realiza a junção dos dados dos records [order] e [order_item] e calcula os totais para cada ordem.
   Os pedidos são filtrados pelos parâmetros [filter_status] e [filter_origin].
   Se os filtros forem strings vazias, nenhum filtro é aplicado.
...
(** 
   ...
   Retorna uma lista de records [aggregated] com os valores médios para cada grupo.
*)
